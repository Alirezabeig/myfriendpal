...
def oauth2callback():
    ...
    if access_token and refresh_token:
        ...
        with conn.cursor() as cursor:  # Using 'with' ensures the cursor will be closed after use
            update_query = '''UPDATE conversations SET oauth_token = %s, google_calendar_email = %s, next_google_calendar_event = %s , local_now = %s , refresh_token = %s WHERE phone_number = %s;'''
            try:
